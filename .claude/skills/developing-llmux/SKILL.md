...
- **"Seen" retires an asked question, but never a detected one.** `mark_thread_seen` (every focus path lands there) clears `@llmux_asking` AND blanks `@llmux_status` so the 🙋 goes on the same paint rather than up to 5s later — but ONLY when `@llmux_status_src` says the `blocked` came from the ask. A permission gate or an unanswered menu is still unanswered whether or not Jay looked at it. The watcher additionally refuses to promote the VISIBLE thread at all: without that the clear is racy, because the watcher works from a `list-panes` snapshot taken before the click and would immediately rewrite the flag it just lost.
...
- **Forge has two distinct representations: tracking links and real sessions.**
  `llmux project forge` remains a link row with no pane behind it; never feed those
  rows into switching, parking, cycling, snapshots, or layout. `llmux resume --tool
  forge` and `llmux add --tool forge --task tsk-…` instead run the installed Forge
  CLI's `task open` inside a real pane, tagged `@llmux_tool=forge` and
  `@llmux_forge_task=<id>`. That task id must survive pause, cycle, refresh, snapshot,
  and restore, and one id may be claimed by only one live or paused pane. Forge's
  checkout is remote, so its local pane cwd is always `$HOME`; never tag or release a
  local worktree/Owner slot for it. The watcher cannot positively identify remote
  OpenCode permission/input state yet, so ordinary `llmux send` refuses Forge unless
  `--force`. The tracking link still renders one row per task above the threads and
  persists under paused.json's `"forge"` map. Its `⚒` glyph is U+2692 Neutral width —
  one column — because link hotspot arithmetic uses character counts.
